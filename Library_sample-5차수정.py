...
class WelcomLibrary:
    ...
    def SearchBook(self):#도서 검색(도서명, 저자, 출판연도, 출판사명, 장르 각각 검색 가능) #Clear
        if Check_Books == [] and self.Plus_Book == []:
            print("현재 저장되어 있는 도서가 없습니다. ")
            self.menu()
        else :
            for i in range(len(Check_Books) + len(self.Plus_Book)):
                if i < len(Check_Books):
                    print(i, '. ', Check_Books[i])
                elif i >= len(Check_Books) :
                    print(i, ', ', self.Plus_Book[i-len(Check_Books)])
        ...
        if SearchBook_selectMenu == 1: #도서명 검색
            ...
            for i in range (len(Check_Books)):
                if Check_Books[i][0] == SearchBook_selectMenu_name :
                    print(Check_Books[i])
            ...
            for i in range (len(Check_Books)):
                if Check_Books[i][1] == SearchBook_selectMenu_writer :
                    print(Check_Books[i])
            ...
            for i in range (len(Check_Books)):
                if Check_Books[i][2] == SearchBook_selectMenu_year :
                    print(Check_Books[i])
            ...
            for i in range (len(Check_Books)):
                if Check_Books[i][3] == SearchBook_selectMenu_company :
                    print(Check_Books[i])
            ...
            for i in range (len(Check_Books)):
                if Check_Books[i][4] == SearchBook_selectMenu_genre :
                    print(Check_Books[i])
    ...
    def RetouchBook(self):#도서 정보 수정 - 수정할 도서명을 입력 받아 입력할 부분을 5가지로 분할하여 고른 뒤 수정 vs 해당 도서명의 내용을 입력받은 내용으로 덮어쓰기
        ...
        if RetouchBook_inputs == 1:
            ...
            if RetouchBook_input < len(Check_Books):
                Check_Books[RetouchBook_input][0] = RetouchBook_bookname
            elif RetouchBook_input >= len(Check_Books):
                self.Plus_Book[RetouchBook_input-len(Check_Books)][0] = RetouchBook_bookname
            ...
            if RetouchBook_input < len(Check_Books):
                Check_Books[RetouchBook_input][1] = RetouchBook_bookwriter
            elif RetouchBook_input >= len(Check_Books) :            
                self.Plus_Book[RetouchBook_input-len(Check_Books)][1] = RetouchBook_bookwriter
            ...
            if RetouchBook_input < len(Check_Books):
                Check_Books[RetouchBook_input][2] = RetouchBook_bookyear
            elif RetouchBook_input >= len(Check_Books) :            
                self.Plus_Book[RetouchBook_input-len(Check_Books)][2] = RetouchBook_bookyear
            ...
            if RetouchBook_input < len(Check_Books):
                Check_Books[RetouchBook_input][3] = RetouchBook_bookcompany
            elif RetouchBook_input >= len(Check_Books) :
                self.Plus_Book[RetouchBook_input-len(Check_Books)][3] = RetouchBook_bookcompany
            ...
            if RetouchBook_input < len(Check_Books):
                Check_Books[RetouchBook_input][4] = RetouchBook_bookgenre
            elif RetouchBook_input >= len(Check_Books) :
                self.Plus_Book[RetouchBook_input-len(Check_Books)][4] = RetouchBook_bookgenre
    ...
    def KillBook(self):#도서 삭제 - 총도서 목록을 출력한 뒤에 책 이름을 입력 받아 리스트의 내용과 비교후 삭제, 해당 도서명이 없을 경우 예외처리
        ...
        if Check_Books != [] or self.Plus_Book != [] :
            num = int(input("\n 삭제할 도서의 번호를 입력해주세요 : "))
            if num < len(Check_Books):
                Check_Books.remove(Check_Books[num])
            elif num >= len(Check_Books):    
                self.Plus_Book.remove(self.Plus_Book[num-len(Check_Books)])
    ...
    def StoreBook(self):#현재 내용 저장
        THIS_FOLDER = os.path.dirname(os.path.abspath(__file__))
        my_file = os.path.join(THIS_FOLDER, 'input.txt')
        self.StoreBook_final = open(my_file, 'w')
        Userinput = str(input("작업 내용을 저장하시겠습니까? (y/n) : "))
        if Userinput == 'y':
            for i in range(len(Check_Books)):
                self.StoreBook_final.writelines(" ".join(Check_Books[i]))

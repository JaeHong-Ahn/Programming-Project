...
class WelcomLibrary:
    ...
    def SearchBook(self):#도서 검색(도서명, 저자, 출판연도, 출판사명, 장르 각각 검색 가능) #Clear
        if Book_line == [] and self.Plus_Book == []:
            print("현재 저장되어 있는 도서가 없습니다. ")
            self.menu()
        else :
            for i in range(len(Book_line) + len(self.Plus_Book)):
                if i < len(Book_line):
                    print(i, '. ', Book_line[i])
                elif i >= len(Book_line) :
                    print(i, ', ', self.Plus_Book[i-len(Book_line)])
    ...
    def RetouchBook(self):#도서 정보 수정 - 수정할 도서명을 입력 받아 입력할 부분을 5가지로 분할하여 고른 뒤 수정 vs 해당 도서명의 내용을 입력받은 내용으로 덮어쓰기
        ...
        if RetouchBook_inputs == 1:
            ...
            if RetouchBook_input < len(Book_line):
                Book_line[RetouchBook_input][0] = RetouchBook_bookname
            else :    
            ...
            if RetouchBook_input < len(Book_line):
                Book_line[RetouchBook_input][1] = RetouchBook_bookwriter
            else :            
            ...
            if RetouchBook_input < len(Book_line):
                Book_line[RetouchBook_input][2] = RetouchBook_bookyear
            else :            
            ...
            if RetouchBook_input < len(Book_line):
                Book_line[RetouchBook_input][3] = RetouchBook_bookcompany
            else :
            ...
            if RetouchBook_input < len(Book_line):
                Book_line[RetouchBook_input][4] = RetouchBook_bookgenre
            else :
    ...
    def KillBook(self):#도서 삭제 - 총도서 목록을 출력한 뒤에 책 이름을 입력 받아 리스트의 내용과 비교후 삭제, 해당 도서명이 없을 경우 예외처리
        ...
        if Book_line != [] or self.Plus_Book != [] :
            num = int(input("\n 삭제할 도서의 번호를 입력해주세요 : "))
            if num < len(Book_line):
                Book_line.remove(Book_line[num])
            elif num >= len(Book_line):    
    ...
    def StoreBook(self):#현재 내용 저장
        ...
        if Userinput == 'y':
            for i in range(len(Book_line) + len(self.Plus_Book)):
                self.StoreBook_final.writelines(" ".join(Book_line[i]))
